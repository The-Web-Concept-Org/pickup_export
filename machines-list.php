...
<html lang="en" dir="ltr">
	...
	<body>
		...
		if (isset($_REQUEST['get'])) {
			$getTypeof='machine_'.$_REQUEST['get'];
			$getInof=$_REQUEST['get'];
			$q=mysqli_query($dbc,"SELECT * FROM machines WHERE `$getTypeof`=".$_REQUEST[$getInof]." ");
			 ...
			 $getTypeof='machine_'.$_REQUEST['gs'];
			$getInof=$_REQUEST['gs'];
			$q=mysqli_query($dbc,"SELECT * FROM machines WHERE `$getTypeof` LIKE '%".$_REQUEST[$getInof]."%' ");
		...
		if (isset($_REQUEST['maker']) AND $_REQUEST['maker']!='null') {
			$makerV="AND machine_maker=".$_REQUEST['maker'];
		}
		if (isset($_REQUEST['brands']) AND $_REQUEST['brands']!='null') {
			$brandV="AND machine_brand=".$_REQUEST['brands'];
		}
		if (isset($_REQUEST['options']) AND $_REQUEST['options']!='null') {
			$optionsV="AND machine_option='".$_REQUEST['options']."'";
		}
		if (isset($_REQUEST['min_price']) AND $_REQUEST['min_price']!='null' AND $_REQUEST['max_price']!='null') {
			$min_price="AND  machine_fob_price BETWEEN ".$_REQUEST['min_price']." AND ".$_REQUEST['max_price'] ;
		}
	
		if (isset($_REQUEST['machine_year_min']) AND $_REQUEST['machine_year_min']!='null' AND $_REQUEST['part_year_max']!='null') {
			$part_year="AND  machine_year BETWEEN ".$_REQUEST['machine_year_min']." AND ".$_REQUEST['machine_year_max'] ;
		}
		if (isset($_REQUEST['transmission']) AND $_REQUEST['transmission']!='null') {
			$transmissionV="AND machine_transmission='".$_REQUEST['transmission']."'";
		}
		if (isset($_REQUEST['fuel']) AND $_REQUEST['fuel']!='null') {
			$fuelV="AND vehicle_fuel='".$_REQUEST['fuel']."'";
		}
			$sq="SELECT * FROM machines WHERE machine_sts=1 ".$makerV." ".$brandV." ".$body_typeV." ".$transmissionV." ".$optionsV." ".$fuelV." ".$part_year." ".$min_price." ";
			//echo $sq;
			$q=mysqli_query($dbc,$sq);

		}else{
				$sq="SELECT * FROM machines WHERE machine_sts=1 ";
				$q=mysqli_query($dbc,$sq);
...
//	$setVeh=getVehicleInfo($dbc,$getV['part_id'],true);
	$maker = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM maker WHERE maker_id = '".$getV['machine_maker']."'"));

$brand = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM brands WHERE brand_id = '".$getV['machine_brand']."' "));
	$img = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM vehicle_images WHERE vehicle_id = '$getV[machine_id]' AND images_type='machine' LIMIT 1"));
												...
												<div class="d-md-flex">
													<div class="item-card9-img">
														...
														<div class="item-card9-imgs">
															<a class="link" href="singlemachine.php?i=<?=base64_encode($getV['machine_id'])?>"></a>
													...
													<div class="card border-0 mb-0">
														<div class="card-body ">
															<div class="item-card9">
																<a href="singlemachine.php?i=<?=base64_encode($getV['machine_id'])?>" class="text-dark"><h4 class="font-weight-semibold mt-1"> <?=$maker['maker_name']?> <?=$brand['brand_name']?> </h4></a>
																<div class="item-card9-desc mb-2">
																	<a href="#" class="mr-4"><span class=""><i class="fa fa-truck text-muted mr-1"></i> <?=ucwords($getV['machine_serial_no'])?></span></a>
																	<a href="#" class="mr-4"><span class=""><i class="fa fa-calendar-o text-muted mr-1"></i>  <?=$getV['machine_timestamp']?></span></a>
																</div>
																<p class="mb-0 leading-tight"><?=$getV['machine_note']?></p>
															</div>
														</div>
														<div class="card-footer pr-4 pl-4 pt-4 pb-4">
															<div class="item-card9-footer d-sm-flex">
																<div class="item-card9-cost">
																	<h4 class="text-dark font-weight-bold mb-0 mt-0">$<?=$getV['machine_fob_price']?></h4>
																</div>
																<div class="ml-auto">
																	<a href="#" class="mr-2" title="Car type"><i class="fa fa-car  mr-1 text-muted"></i>  <?=ucwords($getV['machine_type'])?></a>
																	<a href="#" class="mr-2" title="Steering"><i class="fa fa-road text-muted mr-1 "></i> <?=$getV['machine_steering']?>Km</a>
																	<a href="#" class="" title="FuealType"><i class="fa fa-tachometer text-muted mr-1"></i><?=ucwords($getV['machine_fuel'])?></a>
																</div>
															</div>
														</div>
													</div>
